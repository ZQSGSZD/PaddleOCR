...
def save_structure_res(res, save_folder, image_file):
    print("save_structure_res")
    print(image_file)
    img = cv2.imread(image_file)
    excel_save_folder = os.path.join(save_folder, os.path.basename(image_file)) #连接两个或更多的路径名组件
    os.makedirs(excel_save_folder, exist_ok=True) #创建目录
    # save res
    with open(
...
def main(args):
    ...
    for image_file in image_file_list:
        ...
        # seed http request
        starttime = time.time()
        # data = [{'images': [cv2_to_base64(img)]},{'output': [cv2_to_base64(img)]}]
        # data = {'images': [cv2_to_base64(img)],'pagenum': '1'}

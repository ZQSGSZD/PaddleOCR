...
# 文本图像小角度旋转矫正（边缘投影法）
# 参考文章地址： https://www.jianshu.com/p/e94242f6bee9
class ImgCorrectUtil():
    ...
    def findangle(_image):
        ...
        for rotate in range(-angleThres, angleThres):
            ...
            rotatePoints = ImgCorrectUtil.rotate_points(points, rotateAngle, cX, cY)
    ...
    @classmethod
    def correctImg(cls, filapath, outpath):
        Path = filapath
        cv_img = cv2.imdecode(np.fromfile(Path, dtype=np.uint8), -1)
        cv_img = cv2.cvtColor(cv_img, cv2.COLOR_RGB2BGR)
        angle = ImgCorrectUtil.findangle(cv_img)
        img = ImgCorrectUtil.rotate_bound(cv_img, -angle)
...
#                 2)  # 绘制文字
#     cv2.imwrite('../img/out/test.png', img)
#     print(angle)
